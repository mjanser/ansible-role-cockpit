Vagrant.configure('2') do |config|
  ...
  config.vm.hostname = 'ansible-role-cockpit-fedora-24'
  ...
  config.vm.provision 'shell' do |s|
    ...
    s.inline = <<SCRIPT
sleep 2
curl -s -H "Host: example.com" http://localhost | grep -qi "cockpit" && echo "curl request was successful" || { echo "curl request failed" && exit 1; }
